...
- **Every Answer on the List**: Arbitrary gated questions (affiliation, role, country, intended use, terms) are parsed and shown on the request row itself — no second panel to open. The four answers that decide most reviews take fixed inline slots, the long free-text answer sits beneath them, and any remaining answers expand in place without losing your position in the queue. Requests that left several questions blank are flagged on the row.
