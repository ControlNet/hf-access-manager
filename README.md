...
- **Every Answer on the List**: Arbitrary gated questions (affiliation, role, country, intended use, terms) are parsed and shown on the request row itself — no second panel to open. Every answer is on the row: the ones that decide most reviews lead, the long free-text answer sits beneath them, and only a form long enough to swallow the viewport overflows into an in-place expander that never costs your position in the queue. Requests that left several questions blank are flagged on the row.
